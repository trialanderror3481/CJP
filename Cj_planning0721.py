...
class ActionLayer(BaseActionLayer):
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        # TODO: implement this function
        # 第三段代码开始
        for precond1 in self.parents[actionA]:
            for precond2 in self.parents[actionB]:
                if precond1 in self.parent_layer._mutexes[precond2]:
                    return True
    
        for precond2 in self.parents[actionB]:
            for precond1 in self.parents[actionA]:
                if precond2 in self.parent_layer._mutexes[precond1]:
                    return True
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        # TODO: implement this function
        # 第四段代码开始
        for action1 in self.parents[literalA]:
            for action2 in self.parents[literalB]:
                if action1 in self.parent_layer._mutexes[action2]:
                    return True
        
        for action2 in self.parents[literalB]:
            for aciton1 in self.parent[literalA]:
                if action2 in self.parent_layer._mutexes[action1]:
                    return True
        return False
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        for goal in self.problem.goals:
            ...
            for level in range(len(self.literal_layers)):
                for literal in self.literal_layers[level]:
                    if goal==literal:
                        goal_found=True
                        level_sum+=1
                        break
    ...
    def h_setlevel(self):
        ...
        for layer in self.literal_layers:
            ...
            goalAreMutex=False
            for goalA in self.literal_layers[i]:
                for goalB in self.literal_layers[i]:
                    if goalA!=goalB:
                        if goalA in self.literal_layers[i].is_mutexes[goalA]:
                            goalAreMutex=True
